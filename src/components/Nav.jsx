        ...
        <div className='shadow-md w-full fixed top-0 left-0'>
            <div className='flex  justify-between bg-white dark:bg-[rgb(23,27,63)]  py-4 md:px-10 px-7'>
                ...
                <div onClick={() => setOpen(!open)} className='absolute right-8 top-6 cursor-pointer md:hidden w-7 h-7'>
                    {
                        open ?  < XMarkIcon className="text-[rgb(23,27,63)] dark:text-coral-red" /> :
                        <Bars3BottomRightIcon className="text-[rgb(23,27,63)] dark:text-coral-red" />
                    }
                </div>
                ...
                {/* linke items */}
                <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white dark:bg-[rgb(23,27,63)]  md:z-auto  left-0 w-full md:w-auto md:pl-0 pl-9  gap-6 transition-all duration-100 ease-in ${open ? 'top-[120px]' : 'top-[-490px]'}`}>
                    {navLinks.map((item) => (
